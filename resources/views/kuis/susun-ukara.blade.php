...
@section('konten')
    @if (! $soal)
        <div class="bg-surface-container-lowest rounded-2xl p-10 text-center border border-gray-100 shadow-sm">
            ...
            <h2 class="font-heading text-heading font-bold text-on-surface mt-3">Durung ana soal susun ukara</h2>
    ...
    @else
        <div class="flex flex-col gap-6">
            <section class="flex flex-col gap-2">
                <div class="font-label-upper text-label-upper uppercase tracking-wider text-primary-600 font-bold">Latihan Susun Ukara (FR-4)</div>
            ...
            <section class="bg-surface-container-lowest rounded-2xl p-6 border-2 border-dashed border-primary-400/60 shadow-sm">
                <div class="flex items-center justify-between pb-3 mb-3 border-b border-dashed border-gray-200">
                    <span class="font-label-upper text-label-upper uppercase tracking-wider text-gray-500 font-bold">Ukara kang Diwangun</span>
                    <div class="flex items-center gap-2">
                        <button type="button" id="btn-undo" class="font-caption text-caption font-bold text-gray-500 hover:text-error">Busek Pungkasan</button>
                        <button type="button" id="btn-reset" class="font-caption text-caption font-bold text-gray-500 hover:text-error">Baleni Kabeh</button>
                    </div>
                </div>
                <div id="zona-ukara" class="min-h-[110px] flex flex-wrap items-center gap-2.5 p-3 bg-primary-fixed/30 rounded-2xl border border-gray-100">
                    <span id="placeholder" class="font-body text-body text-gray-500">Klik tembung ing ngisor kanggo nyusun ukara...</span>
                </div>
            </section>
            ...
            <section class="bg-surface-container-lowest rounded-2xl p-6 border border-gray-100 shadow-sm">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <h3 class="font-heading text-heading font-bold text-on-surface">Bank Tembung Pilihan</h3>
                </div>
                ...
                try {
                    ...
                    feedback.innerHTML = `
                        <div class="flex items-center gap-2 font-heading text-heading font-extrabold ${res.benar ? 'text-green-500' : 'text-error'}">
                            ...
                            ${res.benar ? 'Ukara bener!' : 'Durung trep'} • Skor ${res.skor}/100 • +${res.exp_didapat} XP
                        </div>
                        <p class="font-body text-body text-on-surface-variant mt-1">Kunci: <strong>${(res.detail?.kunci ?? []).join(' ')}</strong> • Rekor Skor: ${res.skor_tertinggi}/100 • Total EXP ${res.total_exp} • Streak ${res.current_streak} dina</p>
                        ${levelSelesaiHtml}
                        ${nextButtons}`;
