        ...
        <section class="bg-surface-container-lowest rounded-3xl p-5 sm:p-6 shadow-sm border border-gray-100 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <form method="GET" action="{{ route('superadmin.siswa') }}" class="flex flex-col sm:flex-row sm:items-center gap-3 flex-1">
                ...
                <div class="relative flex-1">
                    ...
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari nama lengkap utawa NIS siswa..."
        ...
        <section class="bg-surface-container-lowest rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                            ...
                            <tr class="hover:bg-surface-container-low/40 transition-colors">
                                        ...
                                        <div class="flex flex-col min-w-0">
                                            <div class="flex items-center gap-2 flex-wrap">
                                                ...
                                                <span class="px-2 py-0.5 rounded-md bg-surface-container-high text-gray-600 font-label-upper text-[10px] font-bold uppercase">
                                                    {{ $s->jenis_kelamin === 'L' ? 'Lanang' : 'Wadon' }}
                                                </span>
                                            </div>
                                ...
                                <td class="py-4 px-6 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form method="POST" action="{{ route('superadmin.siswa.destroy', $s) }}" onsubmit="return confirm('Apa panjenengan yakin pengin mbusak akun siswa {{ $s->nama_lengkap }}?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" title="Busak Akun Siswa"
                                ...
                                <td colspan="5" class="py-12 px-6 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center gap-3">
                                        ...
                                        <div class="flex flex-col gap-1">
                                            <h4 class="font-heading text-heading font-bold text-on-surface">Durung Ana Akun Siswa</h4>
                                            <p class="font-body text-body text-gray-500">Ora ana data siswa sing cocog karo saringan utawa durung didaftarake.</p>
                                        </div>
                                        <div class="pt-2">
                                            <a href="{{ route('superadmin.siswa.create') }}"
            ...
            <div class="px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-surface-container-low/20">
                <div class="text-caption text-gray-500 font-caption">
                    Nampilake <span class="font-bold text-on-surface">{{ $siswaList->firstItem() ?? 0 }}</span> - <span class="font-bold text-on-surface">{{ $siswaList->lastItem() ?? 0 }}</span> saka total <span class="font-bold text-on-surface">{{ $siswaList->total() }}</span> siswa
                </div>
                <div>
                    {{ $siswaList->links() }}
                </div>
