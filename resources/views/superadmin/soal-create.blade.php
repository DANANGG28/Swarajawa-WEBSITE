                    ...
                    <div>
                        ...
                        <p class="font-body text-xs text-gray-500 mt-0.5">
                            @if ($currentPembahasan)
                                Tambahkan soal baru untuk Pembahasan: <span class="font-bold text-primary-700">{{ $currentPembahasan->urutan }}. {{ $currentPembahasan->nama }}</span> (Level {{ $level->urutan }}).
                            @else
                                Tambahkan soal baru untuk Level {{ $level->urutan }}: {{ $level->nama_materi }}.
                            @endif
                        </p>
                    </div>
                </div>

                <a href="{{ route('superadmin.soal', array_filter(['level_materi_id' => $level->id, 'pembahasan_id' => $activePembahasanId])) }}"
